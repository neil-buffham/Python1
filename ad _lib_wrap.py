# Written by Neil Buffham. textwrap module researched on docs.python.org/3/library/textwrap.html
    # This version uses an imported module to wrap the text nicely.
...
#  Combine the seperate outputs into one value:
preformat = (output0 + output1 + output2 + output3 + output4 + output5 + output6 + output7 + output8 + output9 + output10)



postformat = textwrap.fill(preformat, width=125)
print(postformat)