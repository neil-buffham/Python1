...
def detect_border(image):
    ...
    return (border_height >= 3), border_height  # Require at least 3 rows for a valid border
...
for filename in os.listdir(input_folder):
    ...
    if is_image:
        try:
            ...
            if border_detected:
                print(f"{filename[-30:]:<30} {'y':<6} {'y':<15} {border_height:<15} {'crop-output'}")
                total_images_cropped += 1

                # Crop and save images
                cropped_image = image.crop((0, 0, image.width, image.height - border_height - 1))  # Crop one additional row
                ...
                print(f"{filename[-30:]:<30} {'y':<6} {'n':<15} {'n/a':<15} {'crop-none'}")
...
'''
This version kind of aligned the text in the live output, but brought back errors with the pngs and double displays file extensions in the live output.'''