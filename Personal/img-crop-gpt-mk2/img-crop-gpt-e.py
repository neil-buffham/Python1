...
def is_grey_or_brighter(image):
    """
    Check if at least 50% of the pixels in the first four rows from the bottom
    are grey (or brighter).
    """
    width, height = image.size
    grey_count = 0
    total_count = 0

    # Check the bottom four rows
    for y in range(max(0, height - 4), height):
        for x in range(width):
            r, g, b = image.getpixel((x, y))
            # Check if the pixel is grey or brighter
            if (r > 200 and g > 200 and b > 200) or (r + g + b >= 255 * 3 * 0.5):  # Grey or brighter
                grey_count += 1
            total_count += 1

    # Calculate the percentage of grey or brighter pixels
    if total_count > 0 and (grey_count / total_count) >= 0.5:
        return True  # At least 50% grey or brighter
    return False  # Less than 50% grey or brighter
...
def detect_border(image):
    ...
    if border_height < 2:
        ...
        # Add one more row if possible, ensuring we do not exceed image height
        if height - (border_height + 1) >= 0:
            return True, border_height + 1  # Return increased border height
        else:
            return True, border_height  # Return the detected height
...
header = f"{'File Name':<30} {'Image':<6} {'Border Detected':<15} {'Border Height':<15} {'Output Folder'} {'Error Message'}"
print(header)
print('-' * len(header))
...
for filename in os.listdir(input_folder):
    ...
    if is_image:
        try:
            ...
            # Preliminary check for grey or brighter pixels
            if is_grey_or_brighter(image):
                ...
                csv_data.append([filename, 'y', 'n', 'n/a', 'crop-none', error_message])
            ...
            if border_detected:
                ...
                csv_data.append([filename, 'y', 'y', border_height, 'crop-output', error_message])
