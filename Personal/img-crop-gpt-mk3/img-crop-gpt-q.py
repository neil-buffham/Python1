...
def second_scan(image, initial_border_height):
    ...
    fail_black_percentage = 0.0
    fail_yellow_percentage = 0.0
    fail_other_percentage = 0.0
    ...
    for y in range(height - 1, start_y - 1, -1):
        ...
        if black_percentage + yellow_percentage >= 0.75 and yellow_percentage <= 0.45:
            ...
            fail_black_percentage = black_percentage
            fail_yellow_percentage = yellow_percentage
            fail_other_percentage = other_percentage
            break

    return refined_border_height + 1, b2_black_percentage, b2_yellow_percentage, b2_other_percentage, fail_black_percentage, fail_yellow_percentage, fail_other_percentage  # Add fail percentages
...
header = f"{'File Name':<30} {'Image':<6} {'Border1':<10} {'Border1 Height':<15} {'Border2':<10} {'B2 Black':<10} {'B2 Yellow':<10} {'B2 Other':<10} {'Fail Black':<10} {'Fail Yellow':<10} {'Fail Other':<10}"
print(header)
print('-' * len(header))
    ...
    if is_image:
        try:
            ...
            if border1_pass:
                ...
                border2_height, b2_black_percentage, b2_yellow_percentage, b2_other_percentage, fail_black, fail_yellow, fail_other = second_scan(image, initial_border_height)
            ...
            print(f"{truncated_filename:<30} {'y':<6} {border1_result:<10} {initial_border_height:<15} {border2_height:<10} {b2_black_percentage:<10} {b2_yellow_percentage:<10} {b2_other_percentage:<10} {fail_black:<10} {fail_yellow:<10} {fail_other:<10}")

            # Log the data in the CSV report
            csv_data.append([filename, 'y', border1_result, initial_border_height, border2_height, 
                             b2_black_percentage, b2_yellow_percentage, b2_other_percentage, fail_black, fail_yellow, fail_other])

        except Exception as e:
            error_message = str(e)
            shutil.copy(file_path, os.path.join(crop_not_compatible_folder, filename))
            total_non_compatible_copied += 1
            csv_data.append([filename, 'y', 'fail', 'n/a', 'n/a', 'n/a', 'n/a', 'n/a', 'n/a', 'n/a'])

    elif is_video:
        print(f"{filename:<30} {'n':<6} {'n/a':<10} {'n/a':<15} {'n/a':<10} {'n/a':<10} {'n/a':<10} {'n/a':<10} {'n/a':<10} {'n/a':<10} {'n/a':<10}")
        total_videos_copied += 1
        shutil.copy(file_path, os.path.join(crop_none_vids_folder, filename))
        csv_data.append([filename, 'n', 'n/a', 'n/a', 'n/a', 'n/a', 'n/a', 'n/a', 'n/a', 'n/a'])
...
with open(csv_file_path, mode='w', newline='') as csv_file:
    ...
    # Write header
    csv_writer.writerow(['File Name', 'Image', 'Border1', 'Border1 Height', 'Border2', 'B2 Black', 'B2 Yellow', 'B2 Other', 'Fail Black', 'Fail Yellow', 'Fail Other'])
    # Write data rows
    csv_writer.writerows(csv_data)

print(f"CSV report generated at: {csv_file_path}")
'''We are still off of the rails here. Going to shift focus and have gpt generate a program that analyzes the bottom of an image, seperated into two haves.
From this, we can develop thresholds for the two seperate halves of the bottom of the image, and begin the processing code from scratch. I hope to have GPT break
down the concepts and code structure used, but may have to do that on an earlier model, as 245 lines of code is messy and unruly for this project, at least using this approach.'''