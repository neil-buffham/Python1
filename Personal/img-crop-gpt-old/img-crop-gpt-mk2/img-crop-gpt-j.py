    ...
    if is_image:
        try:
            ...
            if border_detected:
                ...
                print(f"{filename[:15]}...{filename[-10:]:<30} {'y':<6} {'y':<15} {border_height:<15} {'crop-output':<15}")
                ...
                print(f"{filename[:15]}...{filename[-10:]:<30} {'y':<6} {'n':<15} {'n/a':<15} {'crop-none':<15}")
        ...
        print(f"{filename:<30} {'n':<6} {'n/a':<15} {'n/a':<15} {'crop-none-vids':<15}")
        ...
        print(f"{filename:<30} {'n':<6} {'n/a':<15} {'n/a':<15} {'crop-not-compatible':<15}")
        total_non_compatible_copied += 1
        shutil.copy(file_path, os.path.join(crop_not_compatible_folder, filename))
...
'''
This version works with pngs and has longer file names corrected in the live view. Shorter names mess up the formatting still, and the headers don't match.'''