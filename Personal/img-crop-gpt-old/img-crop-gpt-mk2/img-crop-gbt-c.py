...
def detect_border(image):
    ...
    for y in range(height - 1, -1, -1):  # Start from the bottom of the image
        ...
        if (black_percentage >= 0.75 and black_percentage <= 0.97) or (black_percentage >= 0.50 and yellow_percentage > 0):  # Adjusted yellow threshold
            border_height += 1
    ...
    return (border_height >= 2), border_height  # Return a boolean for border detection and the height
...
'''This version works fine, but leaves very small remnants of yellow'''